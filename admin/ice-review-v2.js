(() => {
  ...
  function locationOf(story) {
    const payload = story?.ai_payload || {};
    return clean(payload.location_text || [payload.city, payload.state_code].filter(Boolean).join(" · ")) || "地点待确认";
  }

  function sourceLanguageOf(story) {
    const language = clean(story?.ai_payload?.source_language).toLowerCase();
    if (language === "en") return "英文信源 · 已译中文";
    if (language === "mixed") return "中英混合信源";
    if (language === "zh") return "中文信源";
    return "X信源";
  }

  function riskLabels(story) {
    const items = [];
    if (story.conflict_detected) items.push("事实冲突");
    if (story.legal_risk) items.push("法律风险");
    if (story.privacy_risk) items.push("隐私风险");
    if (story.fabrication_risk) items.push("虚构风险");
    return items;
  }
  ...
  reviewApi = async function reviewApiV2(action, payload = {}) {
    if (action === "list" || action === "publish_now") {
      return callV2(action, payload);
    }
    return originalReviewApi(action, payload);
  };

  renderReviewCard = function renderReviewCardV2(story) {
    const risks = riskLabels(story);
    const riskHtml = risks.length
      ? `<span class="risk-chip danger">${risks.map(html).join(" · ")}</span>`
      : `<span class="risk-chip safe">无硬风险</span>`;
    const image = story.cover_image
      ? `<img src="${attr(story.cover_image)}" alt="" loading="lazy" />`
    ...
    return `
      <article class="review-item review-item-v2">
        <div class="review-item-media">${image}</div>
        <div class="review-item-main">
          <div class="review-item-topline">
            <span class="status-pill review-status-${html(story.status)}">${reviewStatusLabel(story.status)}</span>
            ${riskHtml}
            <span class="source-chip">${html(sourceLanguageOf(story))}</span>
            <span class="location-chip">📍 ${html(locationOf(story))}</span>
          </div>
          <h3>${html(title)}</h3>
          <p>${html(bulletin)}</p>
          <div class="score-row">
        ...
        <div class="review-item-action">
          <time>${html(formatDate(story.updated_at || story.last_seen_at))}</time>
          <button onclick="openIceReview('${attr(story.id)}')">查看并审核</button>
        </div>
      </article>
    `;
  };
  ...
  populateReviewModal = function populateReviewModalV2(detail) {
    ...
    const location = locationOf(story);
    const language = sourceLanguageOf(story);

    const grid = el("review-score-grid");
    if (grid) {
      grid.insertAdjacentHTML(
        "beforeend",
        `<div class="review-location-metric"><span>地点分类</span><strong>${html(location)}</strong></div>` +
        `<div><span>信源语言</span><strong>${html(language)}</strong></div>`
      );
    }
    ...
    if (reason) {
      reason.insertAdjacentHTML(
        "afterend",
        `<p class="manual-publish-note">管理员选择“立即发布”后将直接发布；系统只保留风险提示，不再阻止人工决定。</p>`
      );
    }
  };
