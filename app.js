...
async function fetchArticles() {
  let live = [];
  try { live = await fetchLivePublishedArticles(100); } catch (error) { console.warn("Live articles unavailable", error); }
...
function findLeadArticle(categoryArticles, allArticles) {
  return categoryArticles.find((item) => !String(item.image || '').includes('image-placeholder.svg') && String(item.image || '').trim())
    || categoryArticles[0]
    || allArticles[0];
}
