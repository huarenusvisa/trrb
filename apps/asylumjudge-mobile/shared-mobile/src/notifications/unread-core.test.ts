...
import { decrementNotificationUnread, decrementNotificationUnreadBy, normalizeUnreadCounts, unreadBadgeValue, unreadTotal } from './unread-core.ts';
...
test('category reads decrement the exact visible unread count safely', () => {
  assert.deepEqual(decrementNotificationUnreadBy({ notifications: 8, messages: 2 }, 3), { notifications: 5, messages: 2 });
  assert.deepEqual(decrementNotificationUnreadBy({ notifications: 2, messages: 2 }, 9), { notifications: 0, messages: 2 });
  assert.deepEqual(decrementNotificationUnreadBy({ notifications: 2, messages: 2 }, Number.NaN), { notifications: 2, messages: 2 });
});
