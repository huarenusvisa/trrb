...
export default function ArticleDetailScreen() {
  ...
  useEffect(() => {
    ...
    fetchArticle(id)
      .then(setArticle)
      .catch((e) => setError(e instanceof Error ? e.message : '文章加载失败'))
  ...
  if (!article) {
    return (
      <View style={styles.center}>
        <Text style={styles.errorTitle}>暂时无法读取这篇文章</Text>
        <Text style={styles.muted}>{error || '文章可能尚未发布或已经下线。'}</Text>
        <Pressable style={styles.outlineButton} onPress={() => Linking.openURL('https://trrb.net')}><Text style={styles.outlineButtonText}>打开唐人日报网站</Text></Pressable>
      </View>
    );
  }
  ...
  return (
    <>
      <Stack.Screen options={{ title: '新闻详情', headerBackTitle: '返回' }} />
      <ScrollView style={styles.page} contentContainerStyle={styles.content}>
        ...
        <Text style={styles.body}>{article.content || '正文暂不可用。'}</Text>
...
const styles = StyleSheet.create({
  ...
  content:{padding:20,paddingTop:26,paddingBottom:60},
  center:{flex:1,alignItems:'center',justifyContent:'center',padding:28,gap:12},
  muted:{color:'#667085',textAlign:'center'},
  errorTitle:{fontSize:20,fontWeight:'900',color:'#101828'},
  category:{color:'#c8211e',fontWeight:'800'},
  title:{fontSize:30,lineHeight:40,fontWeight:'900',color:'#101828',marginTop:10},
  meta:{color:'#98a2b3',marginTop:12,marginBottom:20},
  image:{width:'100%',height:230,borderRadius:16,backgroundColor:'#eaecf0',marginBottom:22},
  summary:{fontSize:18,lineHeight:29,fontWeight:'700',color:'#344054',marginBottom:20},
  body:{fontSize:18,lineHeight:32,color:'#1d2939'},
  actions:{gap:12,marginTop:30},
  primaryButton:{backgroundColor:'#c8211e',borderRadius:12,paddingVertical:14,alignItems:'center'},
  primaryButtonText:{color:'#fff',fontWeight:'800',fontSize:16},
  outlineButton:{borderWidth:1,borderColor:'#d0d5dd',borderRadius:12,paddingVertical:13,paddingHorizontal:18,alignItems:'center'},
  outlineButtonText:{color:'#344054',fontWeight:'800',fontSize:16}
});
