...
import { appendCreatedCommunityComment, communityCommentDisplayName, communityCommentThreadRootId, paginateCommunityCommentThreads, removeUnpublishedCommunityComment, visibleThreadCountForComment } from '../../src/community/community-comment-presentation';
...
export default function CommunityPostScreen() {
  ...
  useEffect(() => {
    ...
    setExpandedThreads((current) => new Set(current).add(communityCommentThreadRootId(detail.comments, targetCommentId)));
  ...
  useEffect(() => {
    ...
    let active = true;
    let loaded = false;
    setDraftReady(false); setDraftRestored(false); setComment(''); setReplyTarget(null); setVisibleThreadCount(COMMENT_THREAD_PAGE_SIZE); setExpandedThreads(new Set());
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      if (result.comment.parent_id) setExpandedThreads((current) => new Set(current).add(communityCommentThreadRootId(detail.comments.concat(result.comment), result.comment.id)));
      latestComment.current = ''; latestReplyTarget.current = null;
  ...
  const commentPage = paginateCommunityCommentThreads(comments, visibleThreadCount, expandedThreads);
  ...
  return <ScrollView ref={scrollView} testID="community-post-detail" style={styles.page} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor="#c8211e" colors={['#c8211e']} />}>
    ...
    <View style={styles.comments} onLayout={(event) => { commentsOffset.current = event.nativeEvent.layout.y; setTimeout(scrollToTarget, 40); }}>
      ...
      {commentPage.rows.length ? commentPage.rows.map(({ item, depth, replyToLabel, threadRootId, replyCount, expanded }) => <View key={item.id} testID={`community-comment-${item.id}`} onLayout={item.id === targetCommentId ? (event) => { targetCommentOffset.current = event.nativeEvent.layout.y; setTimeout(scrollToTarget, 40); } : undefined} style={[styles.commentCard, depth > 0 && styles.replyCard, item.id === targetCommentId && styles.targetComment, { marginLeft: depth > 0 ? 42 : 0 }]}>
