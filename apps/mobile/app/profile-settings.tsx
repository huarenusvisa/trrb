  ...
  const save = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('update_my_profile', {
        ...
        p_cover_path: nextCoverPath,
        p_is_private: isPrivate,
        p_allow_message_requests: allowMessages,
      });
      if (error) throw error;
      const next = (Array.isArray(data) ? data[0] : data) as SocialProfile | undefined;
      if (!next) throw new Error(t('profileSettings.profileMissing'));
