...
type Profile = { id: string; display_name: string; avatar_key: string; bio: string; };
...
export default function ProfileSettingsScreen() {
  ...
  useEffect(() => { (async () => {
    ...
    else if (data) { const next = data as Profile; setProfile(next); setName(next.display_name); setBio(next.bio || ''); setAvatar(next.avatar_key); }
    setLoading(false);
  })(); }, []);
  ...
  const save = async () => {
    ...
    const { data, error } = await supabase.from('profiles').update({ display_name: trimmedName, bio: trimmedBio, avatar_key: avatar }).eq('id', userId).select('id,display_name,avatar_key,bio').single();
    setSaving(false);
    if (error) {
      const msg = error.message.includes('display_name_reserved') ? '该昵称属于保留身份词，不能使用。' : error.message.includes('duplicate') ? '该昵称已被其他用户使用。' : error.message;
      return Alert.alert('保存失败', msg);
    }
    setProfile(data as Profile); setName(data.display_name); setBio(data.bio || ''); setAvatar(data.avatar_key);
  ...
  return <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.danger}><Text style={styles.dangerTitle}>账户与隐私</Text><Text style={styles.dangerText}>可以永久删除账户及关联个人数据。</Text><Pressable onPress={() => router.push('/delete-account')} style={styles.deleteEntry}><Text style={styles.deleteEntryText}>删除账户</Text></Pressable></View>
...
const styles = StyleSheet.create({
  ...
  h1:{fontSize:30,fontWeight:'900',color:'#101828'},sub:{color:'#667085',marginTop:6,marginBottom:24},avatarRow:{backgroundColor:'#fff',borderRadius:16,padding:18,flexDirection:'row',alignItems:'center',marginBottom:22},avatarControls:{marginLeft:18,flex:1},label:{fontWeight:'800',color:'#344054',marginBottom:8},row:{flexDirection:'row',gap:8},smallButton:{borderWidth:1,borderColor:'#d0d5dd',borderRadius:10,paddingHorizontal:12,paddingVertical:9,backgroundColor:'#fff'},input:{backgroundColor:'#fff',borderWidth:1,borderColor:'#d0d5dd',borderRadius:12,paddingHorizontal:14,paddingVertical:13,fontSize:16,color:'#101828'},bio:{height:120},counter:{textAlign:'right',color:'#98a2b3',marginTop:5,marginBottom:18},save:{backgroundColor:'#c8211e',padding:15,borderRadius:12,alignItems:'center'},disabled:{opacity:.45},saveText:{color:'#fff',fontWeight:'900',fontSize:16},danger:{marginTop:28,paddingTop:22,borderTopWidth:1,borderTopColor:'#d0d5dd'},dangerTitle:{fontWeight:'900',fontSize:18,color:'#101828'},dangerText:{color:'#667085',marginTop:5},deleteEntry:{marginTop:12,borderWidth:1,borderColor:'#b42318',borderRadius:12,padding:13,alignItems:'center'},deleteEntryText:{color:'#b42318',fontWeight:'900'},back:{padding:14,alignItems:'center',marginTop:8},backText:{color:'#475467',fontWeight:'800'}
});
