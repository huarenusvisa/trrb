...
export function CommentThread({ articleId }: { articleId: string }) {
  ...
  return <View testID="news-comments" style={styles.wrap}>
    ...
    {!loading && !loadError && items.length === 0 ? <Text testID="news-comments-empty" style={styles.empty}>暂时还没有评论。</Text> : displayItems.map(({ item, depth, replyToLabel }, index) => <View key={item.id} testID={`news-comment-${index}`} style={[styles.comment, depth > 0 && styles.replyComment]} accessibilityLabel={replyToLabel ? `${item.profiles?.display_name || '唐人读者'}回复${replyToLabel}` : undefined}>
      ...
      {replyToLabel ? <Text style={styles.parentTag}>回复 {replyToLabel}</Text> : null}
