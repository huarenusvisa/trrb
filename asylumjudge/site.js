...
const t = (key, vars = {}) => window.AsylumI18n?.t?.(key, vars) || String(key).replace(/\{(\w+)\}/g, (_, name) => vars[name] ?? '');
...
function renderStateMarket(periods, label, interval) {
  ...
  const intervalLabel = t(interval === 'month' ? '按月' : '按财年');
  const chartLabel = t('{label}趋势图：{interval}显示批准、拒绝及其他裁决。使用左右方向键浏览数据点。', { label, interval: intervalLabel });
  chart.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="group" aria-label="${esc(chartLabel)}" preserveAspectRatio="xMidYMid meet">${grid}${bars}<path class="market-series approval" d="${linePath('approval')}"></path><path class="market-series denial" d="${linePath('denial')}"></path><path class="market-series other" d="${linePath('otherShare')}"></path>${dots}${labels}<line class="market-crosshair" x1="${x(points.length - 1)}" y1="${top}" x2="${x(points.length - 1)}" y2="${bottom}"></line><rect class="market-touch-overlay" x="${left}" y="${top}" width="${width - left - right}" height="${plotHeight}" rx="4"></rect></svg><div class="market-floating-tooltip"></div><p class="market-touch-hint">${esc(t('使用左右方向键浏览数据点；触摸设备可在图表上左右滑动。'))}</p>`;
...
function renderJudgeDirectory(rows, query = '') {
  ...
  container.innerHTML = rows.length ? visibleRows.map((row) => {
    ...
    const webexAction = webex ? `<small class="judge-webex"><a href="${esc(webex.webex_url)}" target="_blank" rel="noopener" aria-label="${esc(t('打开 {judge} 的官方 Webex 出庭链接', { judge: judgeName }))}"><i aria-hidden="true">W</i> Webex 网上上庭 ↗</a><code>${esc(webex.webex_url)}</code><span>电话接入码 ${esc(webex.access_code || '见官方页')}</span></small>` : '';
    const background = row.background_summary;
    const backgroundText = background
      ? [
          background.appointment_date ? `<span lang="en">${esc(background.appointment_date)}</span>任命` : '',
          background.appointment_court ? `<span lang="en">${esc(background.appointment_court)}</span>` : '',
          background.biography_excerpt ? `<span lang="en">${esc(background.biography_excerpt)}</span>` : ''
        ].filter(Boolean).join(' · ')
    ...
    return `<div class="judge-directory-row" role="listitem" data-href="${esc(detailUrl)}"><span class="judge-directory-identity"><a class="judge-profile-link" href="${esc(detailUrl)}"><strong lang="en">${esc(judgeName)}</strong></a><small lang="en">${esc(court)}${place && court !== place ? ` · ${esc(place)}` : ''}</small><small>${esc(judgePeriod(row))}</small><small class="judge-background-summary"><b>法官背景</b>${backgroundText}</small><a class="judge-background-link" href="${esc(detailUrl)}#judge-background" aria-label="${esc(t('查看 {judge} 的法官背景', { judge: judgeName }))}">查看法官背景 →</a>${webexAction}</span><span class="directory-metric"><label>裁决</label><b>${fmt(row.total_asylum_decisions)}</b></span><span class="directory-metric verdict-pass"><label>批准</label><b>${fmt(row.grants)}</b></span><span class="directory-metric verdict-deny"><label>拒绝</label><b>${fmt(row.denials)}</b></span><span class="directory-metric verdict-other"><label>其他</label><b>${fmt(row.other_decisions)}</b></span><span class="directory-metric directory-rate-cell"><label>批准率</label>${rate}<small>${fmt(adjudicated)} 件有效裁决</small><a class="directory-detail-link" href="${esc(detailUrl)}" aria-label="${esc(t('查看 {judge} 的详情', { judge: judgeName }))}">查看详情 →</a></span></div>`;
