(() => {
  ...
  const listingUrl = (item) => `./listing.html?category=${encodeURIComponent(item.name)}`;

  async function fetchCategories() {
    const fields = [
      "id","name","slug","sort_order","is_active","show_in_nav","show_on_home","auto_fetch","ai_rewrite","auto_publish",
  ...
  function normalize(rows) {
    return rows.map((item) => ({
      ...
      showInNav: item.show_in_nav !== false,
  ...
  function publish(channels) {
    ...
    window.TRRB_CHANNELS = channels.filter((item) => item.showOnHome).map((item) => ({
      name: item.name, slug: item.slug, priority: item.priority, enabled: item.enabled, href: item.href
    }));
  ...
  fetchCategories().then((rows) => {
    ...
    const fallback = FALLBACK.map((item, index) => ({...item, priority:Number(item.priority || index + 1), enabled:item.enabled !== false, showInNav:true, showOnHome:true, href:`./listing.html?category=${encodeURIComponent(item.name)}`}));
    publish(fallback);
  });
})();
