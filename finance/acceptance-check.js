(function(){
  ...
  function qaOwned(el){return !!el?.closest?.('#financeQaPanel')}
  function rendered(el){
    if(!el||qaOwned(el)||el.matches?.('input[type="hidden"]'))return false;
    const s=getComputedStyle(el),r=el.getBoundingClientRect(),hiddenAncestor=el.closest('[hidden],[aria-hidden="true"],[inert]');
    return !hiddenAncestor&&s.display!=='none'&&s.visibility!=='hidden'&&s.pointerEvents!=='none'&&r.width>0&&r.height>0
  }
  function operable(el){return rendered(el)&&!el.disabled&&el.getAttribute('aria-disabled')!=='true'}
  ...
  function forbiddenAction(el){
    ...
    if(el.tagName==='BUTTON'||(el.tagName==='INPUT'&&/^(button|submit)$/i.test(el.type||''))){if(actionRe.test(label||own))return label||own}
    if(el.tagName==='A'){
      const contentLink=!!el.closest('.news,.mover,.watch,.fund,.detail-nav,.related');
      if(!contentLink&&own.length<=32&&actionRe.test(own))return own;
    }
    return null
  }
  function badInteractive(viewportOnly=false){const scope=viewportOnly?visible:rendered;return $$('a,button,input[type="button"],input[type="submit"]').filter(el=>scope(el)&&operable(el)).map(forbiddenAction).filter(Boolean).slice(0,16)}
  function targetAudit(viewportOnly=false){
    const limit=window.innerWidth<=620?36:32,scope=viewportOnly?visible:rendered;
    const weak=$$('a,button,input').filter(el=>!el.classList.contains('finance-skip-link')&&scope(el)&&operable(el)).filter(el=>{const r=el.getBoundingClientRect();return r.width<limit||r.height<limit}).slice(0,16).map(el=>`${el.tagName.toLowerCase()}:${text(el).slice(0,18)||el.getAttribute('aria-label')||el.id||'未命名'} ${Math.round(el.getBoundingClientRect().width)}×${Math.round(el.getBoundingClientRect().height)}`);
    return {limit,weak};
  }
  function unnamedInteractive(){return $$('a,button,input,[tabindex="0"]').filter(el=>operable(el)&&!el.classList.contains('finance-skip-link')).filter(el=>!accessibleName(el)).slice(0,16).map(el=>`${el.tagName.toLowerCase()}#${el.id||'-'}.${el.className&&typeof el.className==='string'?el.className.split(/\s+/).filter(Boolean).slice(0,2).join('.'):''}`)}
