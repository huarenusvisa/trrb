(function(){
  ...
  const links=$$('a',nav);
  links.forEach(a=>a.addEventListener('click',e=>{e.preventDefault();const target=$(`#${CSS.escape(a.getAttribute('href').slice(1))}`);if(!target)return;target.scrollIntoView({behavior:document.documentElement.classList.contains('reduce-motion')?'auto':'smooth',block:'start'});history.replaceState(null,'',`${location.pathname}${location.search}${a.getAttribute('href')}`)}));
  ...
  const stockPrice=$('#topPrice'),stockSymbol=$('#topSymbol'),fundSymbol=$('#fundSymbol');
  let basePrimary='',baseSecondary='';
  const ensureFundHeader=()=>{
    if(!isFund)return;
    const price=$('#fundPrice')?.textContent||'—',change=$('#fundChange')?.textContent||'';fundSymbol.textContent=price;
    const span=$('span',ticker);if(span)span.innerHTML=`${$('#fundName')?.textContent||''} <em class="mini-change ${($('#fundChange')?.classList.contains('down'))?'down':'up'}">${change}</em>`;
  };
  const captureBase=()=>{if(isFund){ensureFundHeader();basePrimary=fundSymbol?.textContent||'';baseSecondary=$('span',ticker)?.innerHTML||''}else{basePrimary=stockPrice?.textContent||'';const change=$('#move')?.textContent||'';const sign=$('#move')?.classList.contains('down')?'down':'up';if(stockSymbol){const sym=stockSymbol.textContent;stockSymbol.innerHTML=`${sym} <em class="mini-change ${sign}">${change.match(/\([^)]*\)/)?.[0]||''}</em>`}baseSecondary=stockSymbol?.innerHTML||''}};
  requestAnimationFrame(()=>requestAnimationFrame(captureBase));
  window.addEventListener('finance:chartread',e=>{const d=e.detail||{};ticker.classList.add('chart-reading');if(isFund){if(fundSymbol)fundSymbol.textContent=d.price||basePrimary;const span=$('span',ticker);if(span)span.textContent=`${d.range||''} · ${$('#fundName')?.textContent||''}`}else{if(stockPrice)stockPrice.textContent=d.price||basePrimary;if(stockSymbol)stockSymbol.textContent=`${d.range||''} · ${$('#symbol')?.textContent||''}`}});
  window.addEventListener('finance:chartreadend',()=>{ticker.classList.remove('chart-reading');if(isFund){if(fundSymbol)fundSymbol.textContent=basePrimary;const span=$('span',ticker);if(span)span.innerHTML=baseSecondary}else{if(stockPrice)stockPrice.textContent=basePrimary;if(stockSymbol)stockSymbol.innerHTML=baseSecondary}});
})();
