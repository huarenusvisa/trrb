(function(){
  ...
  function initWatchSorting(){
    ...
    function updateHead(){
      const rows=$$('.watch',list);const manage=$$('.watch-manage-item',list);const listMode=rows.length>0&&!manage.length;head.hidden=!listMode;
    ...
    let applying=false;
    function applySort(){
      if(applying)return;const candidates=[...list.children].filter(n=>n.matches('.watch,.watch-manage-item'));updateHead();if(candidates.length<2)return;
  ...
  function initStockReferenceFeatures(){
    ...
    function normalizedSeries(rangeKey){
      ...
      const raw=D.spark(q.symbol)||[];const n={1W:18,1M:26,3M:32,YTD:38,1Y:44,5Y:52}[rangeKey]||26,spread=spreadMap[rangeKey]||.08;
    ...
    function periodStats(rangeKey){
      if(rangeKey==='1D')return {amount:q.price-q.prev,pct:q.change};const p=normalizedSeries(rangeKey),start=p[0]||q.price,amount=q.price-start,pct=start?amount/start*100:0;return {amount,pct}
    }
    ...
    function candlesFor(rangeKey){
      ...
      return closes.map((close,i)=>{const prev=i?closes[i-1]:close*(1-(q.change||0)/100*.15);const open=prev*(1+Math.sin((i+3)*1.7)*.0035);const wiggle=Math.max(close*.003,Math.abs(close-open)*.42);const high=Math.max(open,close)+wiggle*(1+(i%3)*.25),low=Math.max(.01,Math.min(open,close)-wiggle*(.8+(i%4)*.18));const volume=volBase*(.55+((i*17+q.symbol.length*11)%47)/50);return {open,high,low,close,volume}})
    }
    function renderKline(){
      const rangeKey=activeRange(),candles=candlesFor(rangeKey),w=800,h=400,priceBottom=318,volTop=332,volBottom=386;const max=Math.max(...candles.map(c=>c.high)),min=Math.min(...candles.map(c=>c.low)),maxVol=Math.max(...candles.map(c=>c.volume));const xStep=w/Math.max(1,candles.length),bodyW=Math.max(3,Math.min(14,xStep*.56));const y=v=>24+(max-v)/(max-min||1)*(priceBottom-42);const pieces=[];
