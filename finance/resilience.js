(function(){
  ...
  const isDetail=!!$('.stock-app');let ready=false,recovery=null,recoveryTimer=0;
  ...
  function markReady(){if(ready||!coreReady())return;ready=true;clearTimeout(recoveryTimer);main.setAttribute('aria-busy','false');loadingLabel.textContent='财经内容已加载';document.documentElement.classList.add('finance-app-ready');if(recovery)recovery.remove()}
  ...
  function watchEmpty(selector,kind){const el=$(selector);if(!el)return;const sync=()=>addEmptyAction(el,kind);sync();if('MutationObserver'in window)new MutationObserver(sync).observe(el,{childList:true,subtree:false})}
  watchEmpty('#watchlist','watch');watchEmpty('#fundList','fund');watchEmpty('#historyList','history');watchEmpty('#alertList','alerts');
  const syncReady=()=>requestAnimationFrame(markReady);if('MutationObserver'in window){new MutationObserver(syncReady).observe(document.documentElement,{attributes:true,attributeFilter:['class']});const indices=$('#indices'),status=$('#dataStatus');if(indices)new MutationObserver(syncReady).observe(indices,{childList:true});if(status)new MutationObserver(syncReady).observe(status,{childList:true,subtree:true,characterData:true})}
  ...
  const failWhileLoading=()=>{if(!ready)setTimeout(()=>{if(!ready)showRecovery()},300)};window.addEventListener('error',failWhileLoading);window.addEventListener('unhandledrejection',failWhileLoading);
