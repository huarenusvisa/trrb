(function(){
  ...
  function chart(points){const max=Math.max(...points),min=Math.min(...points),w=800,h=400;const pts=points.map((v,i)=>`${(i/(points.length-1))*w},${h-30-((v-min)/(max-min||1))*(h-80)}`).join(' ');const last=pts.split(' ').slice(-1)[0].split(',')[1];return `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-label="${q.symbol} 价格走势图"><line x1="0" y1="${h*.72}" x2="${w}" y2="${h*.72}" stroke="#a8afb3" stroke-width="1" stroke-dasharray="3 5"/><polyline points="${pts}" fill="none" stroke="${q.change>=0?'#00c805':'#ff4d2e'}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="${w}" cy="${last}" r="6" fill="${q.change>=0?'#00c805':'#ff4d2e'}"/></svg>`}
