...
function renderLoadError(scope) {
  ...
  $('#compare-empty').innerHTML = `<div class="compare-error" role="alert"><p>${esc(words().error)}</p><button type="button" data-retry="${scope}">${esc(words().retry)}</button></div>`;
}
...
$('#compare-empty').addEventListener('click', (event) => {
  ...
  if (!retry) return;
  if (retry.dataset.retry === 'details') loadDetails();
  else load();
});
