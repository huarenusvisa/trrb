...
async function search(query) {
  ...
  try {
    const response = await fetch(`/.netlify/functions/immigration-judges?q=${encodeURIComponent(query)}`);
    ...
    $('#results').innerHTML = rows.map((row) => `<a class="judge-row" href="/immigration-judge-approval-rate/detail.html?id=${encodeURIComponent(row.id)}"><div><div class="name">${esc(row.judge_name)}</div><small>${esc([row.court_city, row.court_state].filter(Boolean).join(', '))}</small></div><div><label>任职法院</label>${esc(row.court_name || '—')}</div><div><label>裁决批准率</label><span class="rate">${row.adjudicated_approval_rate == null ? '—' : `${Number(row.adjudicated_approval_rate).toFixed(1)}%`}</span></div><div><label>拒绝</label><span class="verdict-deny">${fmt(row.denials)}</span></div><div><label>其他</label><span class="verdict-other">${fmt(row.other_decisions)}</span></div></a>`).join('');
  } catch {
    $('#result-note').textContent = '查询暂不可用';
    $('#results').innerHTML = '<div class="empty"><b>数据库接口暂时无法读取</b><p>请稍后重试。</p></div>';
  }
}
