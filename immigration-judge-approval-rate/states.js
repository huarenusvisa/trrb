...
async function load() {
  try {
    ...
    if (initial) $('#state-q').value = initial;
    render(initial ? rows.filter((row) => String(row.state || '').toUpperCase() === initial.toUpperCase()) : rows);
  } catch {
