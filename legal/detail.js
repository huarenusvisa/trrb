(()=>{
  ...
  function cleanField(v){return String(v??'').normalize('NFKC').replace(/\s+/g,' ').trim()}
  function displayDate(v){if(!v)return'日期未提取';const d=new Date(v);if(Number.isNaN(d.getTime()))return cleanField(v);const y=d.getUTCFullYear(),m=String(d.getUTCMonth()+1).padStart(2,'0'),day=String(d.getUTCDate()).padStart(2,'0');return `${y}-${m}-${day}`}
  function displayBody(r){return cleanField(r.issuingBody)||labels[r.sourceSystem]||'发布机构未提取'}
  function displayDocket(v){const s=cleanField(v);return s||'案号未提取'}
  function displayCitation(v){const s=cleanField(v);return s||'正式引证未提取'}
  function titleOf(r){return cleanField(r.title)||cleanField(r.citation)||cleanField(r.docket)||`${labels[r.sourceSystem]||r.sourceSystem}资料`}
  ...
  async function load(){
    ...
    try{
      ...
      $('#detail-type').textContent=cleanField(r.authorityType)||'法律资料';
      $('#detail-title').textContent=title;
      const meta=[displayBody(r),displayDate(r.publicationDate),r.docket?`案号 ${cleanField(r.docket)}`:'',r.citation?displayCitation(r.citation):''].filter(Boolean);
      ...
      $('#detail-fields-list').innerHTML=[
        pair('来源系统',labels[r.sourceSystem]||r.sourceSystem),pair('发布机构',displayBody(r)),pair('资料类型',cleanField(r.authorityType)||'法律资料'),pair('发布日期',displayDate(r.publicationDate)),pair('案号',displayDocket(r.docket)),pair('正式引证',displayCitation(r.citation)),pair('管辖范围',cleanField(r.jurisdiction)),pair('先例状态',cleanField(r.precedentialStatus)),pair('来源键',cleanField(r.sourceKey)),pair('数据库版本',cleanField(db.datasetVersion))
      ].join('');
