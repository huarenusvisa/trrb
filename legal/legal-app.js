(()=>{
  ...
  function writeUrl(){const p=new URLSearchParams();if(state.q)p.set('q',state.q);if(state.source)p.set('source',state.source);if(state.body)p.set('body',state.body);if(state.type)p.set('type',state.type);if(state.page>1)p.set('page',String(state.page));const next=`${location.pathname}${p.toString()?`?${p}`:''}`;history.replaceState(null,'',next)}
  ...
  function bind(){let timer;$('#legal-q').addEventListener('input',e=>{clearTimeout(timer);timer=setTimeout(()=>{state.q=e.target.value.trim();state.page=1;render()},180)});$('#legal-source').addEventListener('change',e=>{state.source=e.target.value;state.body='';state.type='';state.page=1;syncFilters();render()});$('#legal-body').addEventListener('change',e=>{state.body=e.target.value;state.page=1;render()});$('#legal-type').addEventListener('change',e=>{state.type=e.target.value;state.page=1;render()});$('#legal-reset').addEventListener('click',()=>{state.q=state.source=state.body=state.type='';state.page=1;$('#legal-q').value='';$('#legal-source').value='';syncFilters();render()});document.querySelectorAll('.source-tabs button').forEach(b=>b.addEventListener('click',()=>{state.source=b.dataset.source||'';state.body='';state.type='';$('#legal-source').value=state.source;state.page=1;syncFilters();render()}));document.querySelectorAll('[data-circuit-body]').forEach(b=>b.addEventListener('click',()=>{state.source='US_CIRCUIT';state.body=b.dataset.circuitBody||'';state.type='';state.page=1;$('#legal-source').value='US_CIRCUIT';syncFilters(state.body,'');render()}));$('#legal-prev').addEventListener('click',()=>{if(state.page>1){state.page--;render();scrollTo({top:document.querySelector('.status-row').offsetTop-90,behavior:'smooth'})}});$('#legal-next').addEventListener('click',()=>{const pages=Math.max(1,Math.ceil(filtered().length/PAGE_SIZE));if(state.page<pages){state.page++;render();scrollTo({top:document.querySelector('.status-row').offsetTop-90,behavior:'smooth'})}})}
  async function load(){try{const [dbRes,aiRes]=await Promise.all([fetch('/data/legal/unified-legal-authorities-latest.json',{cache:'no-store'}),fetch('/data/legal/legal-ai-analysis-latest.json',{cache:'no-store'})]);if(!dbRes.ok)throw new Error(`数据库HTTP ${dbRes.status}`);const db=await dbRes.json();state.records=Array.isArray(db.records)?db.records:[];state.version=db.datasetVersion||'';if(aiRes.ok){const ai=await aiRes.json();state.analysis=new Map((ai.analyses||[]).map(a=>[a.recordId,a]))}const allowed=new Set(['','SCOTUS','US_CIRCUIT','BIA','WHITE_HOUSE','FEDERAL_REGISTER']);if(!allowed.has(state.source))state.source='';$('#legal-q').value=state.q;$('#legal-source').value=state.source;syncFilters(state.body,state.type);render()}catch(e){$('#legal-count').textContent='数据库加载失败';$('#legal-list').innerHTML=`<div class="empty">暂时无法加载法律数据库：${esc(e.message)}。请稍后重试。</div>`}}
  bind();load();
})();
