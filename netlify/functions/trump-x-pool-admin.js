...
const sourceText = (value) => safeText(value, Infinity).replace(/https?:\/\/\S+/gi, " ").replace(/(?:^|\s)@[A-Za-z0-9_]+/g, " ").replace(/\s+/g, " ").trim();
...
function editorFields(input) { return { title: safeText(input.title, Infinity), summary: safeText(input.summary, Infinity), content: safeText(input.content, Infinity), cover_image: safeText(input.cover_image, 2000) }; }
...
async function publish(row, fields, input, user) {
  const count = length(fields.content); const payload = parsePayload(row.ai_payload);
