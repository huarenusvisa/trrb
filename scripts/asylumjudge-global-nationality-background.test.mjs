...
assert.match(detailPage, /id="country-filter"[^>]*name="nationality"[^>]*type="search"[^>]*inputmode="search"[^>]*autocomplete="off"[^>]*aria-label="输入中文或英文国籍，例如：古巴、Cuba、中国、China"[^>]*aria-controls="nationality"[^>]*aria-describedby="nationality-period-label nationality-results-status"/, 'judge nationality filter must expose a persistent localized name and its result relationship');
...
assert.match(detailClient, /detailLoading\.textContent = detailLoadCopy\(\)\.missing/, 'missing judge IDs must use the active locale');
assert.match(detailClient, /const message = detailLoadCopy\(\);[\s\S]*esc\(message\.unavailable\)[\s\S]*esc\(message\.retryLater\)[\s\S]*esc\(message\.retry\)/, 'judge detail failures and retry controls must render escaped active-locale messages');
assert.doesNotMatch(detailClient, /detailLoading\.textContent = '缺少法官编号'|detailLoading\.innerHTML = '<b>暂时无法读取该法官资料/, 'dynamic judge detail recovery must not remain hard-coded in Simplified Chinese');
for (const locale of ['en', 'es', 'fr', 'pt-BR', 'hi', 'zh-Hans', 'zh-Hant', 'ru', 'ar', 'tr']) {
...
for (const locale of ['en', 'es', 'fr', 'pt-BR', 'hi', 'zh-Hans', 'zh-Hant', 'ru', 'ar', 'tr']) {
  assert.match(nationalityPeriodMessages, new RegExp(`['"]?${locale}['"]?\\s*:`), `nationality period descriptions must support ${locale}`);
}
assert.match(detailClient, /const template = nationalityPeriodMessages\[locale\] \|\| nationalityPeriodMessages\['zh-Hans'\]/, 'nationality periods must use the active locale with a safe fallback');
assert.match(detailClient, /\.replace\('\{year\}', String\(year\)\)[\s\S]*\.replace\('\{start\}', start\)[\s\S]*\.replace\('\{end\}', end\)/, 'localized nationality periods must interpolate the fiscal year date range');
assert.doesNotMatch(detailClient, /return `FY \${year}：/, 'the dynamic nationality period must not remain hard-coded in Simplified Chinese');
const nationalityTableLabels = detailClient.match(/const nationalityTableLabels = \{[\s\S]*?\n\};/)?.[0] || '';
for (const locale of ['en', 'es', 'fr', 'pt-BR', 'hi', 'zh-Hans', 'zh-Hant', 'ru', 'ar', 'tr']) {
  assert.match(nationalityTableLabels, new RegExp(`['"]?${locale}['"]?\\s*:`), `nationality table headers must support ${locale}`);
}
assert.match(detailClient, /const labels = nationalityTableLabels\[locale\] \|\| nationalityTableLabels\['zh-Hans'\]/, 'nationality table headers must use the active locale with a safe fallback');
...
assert.match(detailClient, /const messages = nationalityRowMessages\[locale\] \|\| nationalityRowMessages\['zh-Hans'\]/, 'nationality sample guidance must use the active locale with a safe fallback');
assert.match(detailClient, /const template = count < 50 \? messages\.insufficient : count < 200 \? messages\.limited : messages\.sufficient/, 'nationality sample guidance must preserve the existing reliability thresholds');
assert.match(detailClient, /const template = \(nationalityRowMessages\[locale\] \|\| nationalityRowMessages\['zh-Hans'\]\)\.dates/, 'nationality record dates must use the active locale with a safe fallback');
...
assert.match(page, /id="country-search-form"[^>]*role="search"[\s\S]*id="country-search"[^>]*name="country"[^>]*type="search"[^>]*inputmode="search"[^>]*enterkeyhint="search"[^>]*autocomplete="off"[^>]*spellcheck="false"/, 'nationality search must expose a search landmark and mobile search-key semantics');
...
runInNewContext(`${appointmentDateHelpers}; spanishDate = formatAppointmentDate('July 2017'); window.AsylumI18n.locale = 'zh-Hans'; chineseDate = formatAppointmentDate('April of 2026'); rawDate = formatAppointmentDate('Spring 2020');`, appointmentDateSandbox);
assert.equal(appointmentDateSandbox.spanishDate, 'julio de 2017', 'English appointment months must be localized for Spanish readers');
...
assert.match(detailClient, /return webexMessages\[locale\] \|\| webexMessages\['zh-Hans'\]/, 'Webex hearing guidance must use the active locale with a safe fallback');
assert.match(detailClient, /function renderWebex\(webex\)[\s\S]*const copy = webexCopy\(\)[\s\S]*esc\(copy\.title\)[\s\S]*esc\(item\.court_name \|\| copy\.link\)[\s\S]*esc\(copy\.phone\)[\s\S]*esc\(copy\.accessCode\)[\s\S]*esc\(item\.access_code \|\| copy\.officialPage\)[\s\S]*esc\(copy\.warning\)/, 'Webex hearing details must safely render all active-locale guidance');
assert.doesNotMatch(detailClient, /container\.innerHTML[^\n]*EOIR Webex 网上上庭入口|item\.court_name \|\| 'Webex 网上上庭'|<small>电话 \$\{esc\(webex/, 'Webex hearing rendering must not remain hard-coded in Simplified Chinese');
