...
const backgroundDirectory = readFileSync('asylumjudge/judge-backgrounds.html', 'utf8');
...
assert.match(standalone, /site\.css\?v=40/, 'standalone homepage must load the official-tools navigation stylesheet');
assert.match(trrb, /site\.css\?v=40/, 'embedded homepage must load the official-tools navigation stylesheet');
assert.match(backgroundDirectory, /site\.css\?v=40/, 'background directory must load the current navigation and chart-focus stylesheet');
...
assert.match(styles, /\.search-box \.search-clear\{[^}]*width:44px[^}]*min-width:44px[^}]*height:44px[^}]*touch-action:manipulation/, 'the clear-search control must retain a mobile-sized touch target');
...
assert.match(styles, /\.judge-background-link\{[^}]*display:inline-flex[^}]*min-height:44px[^}]*touch-action:manipulation/, 'judge background actions must provide 44px touch targets');
assert.match(styles, /\.judge-profile-link:focus-visible,\.judge-background-link:focus-visible,\.directory-detail-link:focus-visible,\.judge-webex a:focus-visible\{[^}]*outline:3px solid #101828[^}]*outline-offset:3px/, 'judge directory links must expose a high-contrast keyboard focus indicator');
...
for (const html of [standalone, trrb]) {
  ...
  assert.match(html, /id="judge-directory-list"[^>]+role="list"[^>]+aria-labelledby="all-judges-title"[^>]+aria-busy="true"/, 'judge directory must expose a named list and its initial loading state');
  ...
  assert.match(html, /<ul id="state-list"[^>]+aria-labelledby="state-list-title"[^>]+aria-busy="true"/, 'state overview must expose a named semantic list and its initial loading state');
  assert.equal((html.match(/<li class="skeleton"><\/li>/g) || []).length, 3, 'initial state loading placeholders must preserve valid list structure');
...
assert.match(standalone, /app-i18n\.js\?v=16[\s\S]*site\.js\?v=50/, 'the standalone homepage must load the clear-search client');
assert.match(trrb, /app-i18n\.js\?v=16[\s\S]*site\.js\?v=50/, 'the embedded homepage must load the clear-search client');
...
assert.doesNotMatch(client, /<a class="state-row"[^>]*role="listitem"/, 'state result links must keep their native link role');
assert.match(styles, /\.state-list\{margin:0;padding:0;list-style:none\}/, 'the semantic state list must reset browser list spacing without changing its layout');
...
for (const html of [standalone, trrb]) assert.match(html, /id="judge-search-clear"[^>]+class="search-clear"[^>]+type="button"[^>]+aria-label="清除搜索"[^>]+hidden[^>]*><span aria-hidden="true">×<\/span>/, 'each homepage must provide a hidden, accessible clear-search control');
assert.match(i18n, /\['清除搜索'(?:,'[^']*'){9}\]/, 'clear-search labeling must be available in all ten languages');
assert.match(client, /judge-search-clear'\)\.addEventListener\('click'[\s\S]*judge-q'\)\.value = ''[\s\S]*applyJudgeFilter\('', \{ pushHistory: true \}\)[\s\S]*judge-q'\)\.focus\(\)/, 'clearing search must restore the full directory, update history, and return focus');
assert.match(client, /judge-search-clear'\)\.addEventListener\('click'[\s\S]*url\.searchParams\.delete\('q'\)[\s\S]*history\.pushState/, 'clearing during the initial load must still remove the query from browser history');
assert.match(client, /judge-search-clear'\)\.hidden = !query/, 'the clear-search control must track whether a query is active');
...
assert.match(client, /const normalizeSearchText = \(value\)[\s\S]{0,220}normalize\('NFKD'\)[\s\S]{0,120}replace\(\/\[’'\.,-\]\/g, ''\)/, 'judge search must ignore common omitted name punctuation');
assert.match(client, /const terms = normalizeSearchText\(query\)[\s\S]{0,300}const searchable = normalizeSearchText\(/, 'judge search must normalize both the query and directory values');
...
assert.match(standalone, /app-i18n\.js\?v=16[\s\S]*site\.js\?v=50/, 'standalone homepage must load the clear-search client');
assert.match(trrb, /app-i18n\.js\?v=16[\s\S]*site\.js\?v=50/, 'embedded homepage must load the clear-search client');
