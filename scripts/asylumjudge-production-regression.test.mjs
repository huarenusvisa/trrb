...
const uscisPageSnapshot = readFileSync('asylumjudge/uscis-asylum-snapshot.js', 'utf8');
...
assert.match(uscisPageSnapshot, /window\.USCIS_ASYLUM_SNAPSHOT\s*=\s*\{/, 'USCIS page must ship its official snapshot as executable static data');
assert.ok((uscisPageSnapshot.match(/"office":/g) || []).length >= 30, 'USCIS page snapshot must include every published office-period row');
...
for (const line of redirects.split('\n')) {
  ...
  if (/^30[1278]!?$/.test(status || '')) {
    assert.notEqual(from.replace(/\/+$/, ''), to.replace(/\/+$/, ''), `Redirect must not loop between slash variants: ${line}`);
  }
}
for (const locale of locales) {
