...
const rewriteHrefPrefix = (html, from, to) => html.replace(
  new RegExp(`href="${escapeRegex(from)}(?=["?#])`, 'g'),
  `href="${to}`
);
...
async function resolvesInternalHref(href) {
  ...
  if (await exists(join(direct, 'index.html'))) return true;
...
console.log(`AsylumJudge standalone link hygiene: ${changedFiles} HTML files changed; ${replacements} broken/legacy hrefs rewritten; 0 unresolved internal routes.`);
