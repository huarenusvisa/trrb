...
check('settings expose pending token sync without leaking its token', registration.includes('getPendingPushRegistrationStatus') && registration.includes('pending.userId !== auth.user.id') && settings.includes('push-registration-retry') && settings.includes('AccessibilityInfo.announceForAccessibility'));
