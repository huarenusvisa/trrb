...
const registrationApi = read('apps/mobile/src/push/registration-api.ts');
...
check('token ownership is claimed only through authenticated server identity', registration.includes('claimPushToken') && registrationApi.includes('Authorization: `Bearer ${options.accessToken}`') && tokenClaimEndpoint.includes('authenticateUser(event)') && tokenClaimEndpoint.includes('p_user_id: user.id'));
check('same physical token has one transactionally serialized active owner', tokenClaimMigration.includes('pg_advisory_xact_lock') && tokenClaimMigration.includes('user_id <> p_user_id') && tokenClaimMigration.includes('security invoker') && tokenClaimMigration.includes('to service_role'));
...
check('failed token registration persists bounded account-scoped retry state', registration.includes('PENDING_REGISTRATION_KEY') && registration.includes('nextPendingPushRegistration') && registration.includes('pendingPushRetryDelay') && registration.includes("state === 'active'"));
check('network recovery retries pending registration without duplicate mutations', pkg.dependencies?.['expo-network'] === '~57.0.1' && registration.includes('Network.addNetworkStateListener') && registration.includes('connectivityGate.record(state)') && registration.includes('pendingRetryInFlight'));
check('settings expose account-scoped pending token diagnostics without leaking its token', registration.includes('getPendingPushRegistrationStatus') && registration.includes('pending.userId !== userId') && registration.includes('errorKind: pending.errorKind') && settings.includes('push-registration-error-kind') && settings.includes('push-registration-next-retry') && settings.includes('push-registration-retry') && settings.includes('AccessibilityInfo.announceForAccessibility'));
