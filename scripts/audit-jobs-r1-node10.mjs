...
// Full gate intentionally re-runs every predecessor audit against current main after each repair.
...
const spec=read('docs/JOBS-01-US-RECRUITING-AND-JOB-SEEKING.md');const n1=read('supabase/migrations/20260817001000_jobs_r1_node1_foundation.sql');const n2=read('supabase/migrations/20260817002000_jobs_r1_node2_home_admin_migration.sql');const n8=read('supabase/migrations/20260817008000_jobs_r1_node8_reviews_reports.sql');const n9=read('supabase/migrations/20260817009000_jobs_r1_node9_lifecycle_seo_governance.sql');const mobile=read('apps/mobile/app/jobs.tsx');const feed=read('netlify/functions/public-jobs.js');const webSearch=read('jobs/search.js');const webPublish=read('jobs/publish.js');const admin=read('admin/jobs-manager.js')+read('admin/jobs-lifecycle.js')+read('admin/jobs-seeker-privacy.js');const listing=read('jobs/listing.html')+read('jobs/listing.js');
const checks=[
 ...
 ['admin same canonical source',/job_listings/.test(admin)&&/job_seeker_posts/.test(admin)&&/job_lifecycle_events/.test(admin)&&/job_seeker_profiles/.test(admin)],
