...
for (const token of ['p_keyword','p_category_slug','p_employment_type','p_state_code','p_city','p_county','p_borough','p_neighborhood','p_salary_min']) must(sql.includes(token), `search RPC missing ${token}`);
for (const sort of ["p_sort='distance'","p_sort='latest'","p_sort='salary'","p_sort='relevance'"]) must(sql.includes(sort), `search RPC missing ${sort}`);
...
must(js.includes('navigator.geolocation') && (js.includes('未获得定位授权，可继续手动选择州、城市或社区') || js.includes('未获得定位授权，可继续手动选择州、城市、County/Borough 或 Neighborhood')), 'distance sort must be optional and manual hierarchical location must remain usable');
...
must(admin.includes("supabaseClient.from(table).update(") && admin.includes('data-jobs-status'), 'unified admin must govern lifecycle status');
