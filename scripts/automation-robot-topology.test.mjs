...
test("only the operations master remains scheduled", () => {
  const scheduled = fs.readdirSync(workflowsDir)
    .filter((name) => name.endsWith(".yml"))
    .filter((name) => /^  schedule:/m.test(readWorkflow(name)))
    .sort();
  assert.deepEqual(scheduled, ["operations-control-plane.yml"]);
});

test("legacy 404 audit is manual-only, one-shot and report-only", () => {
  ...
  assert.match(workflow, /legacy_404 one-shot control reset to false/);
});
