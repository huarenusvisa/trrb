...
export async function researchEvent(qualified, tweet, {request, readJson, model, key, bearer, editorialDepth = 'standard', analysisAngles = []}) {
  ...
  return {text: research?.text || '没有检索到可靠的外部背景资料', web_search_completed: research?.web_search_completed === true, thread, thread_status: threadError || (thread.length ? '已取得原帖公开讨论，观点不构成事实佐证' : '原帖暂无可用公开讨论'), sources: research?.sources || []};
}
