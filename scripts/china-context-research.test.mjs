...
test('obsolete database length failures retry once; political and fact-check holds do not',()=>{
 ...
 assert.equal(contextRetryEligible({...row,ai_payload:{...row.ai_payload,processing_version:'unified-news-research-2000-3500-v2'}},now),false);
...
test('only recent automated length holds retry once; manual, old and current-version holds stay',()=>{
  ...
  assert.equal(contextRetryEligible({...row,ai_payload:{quality_hold:true,processing_version:'unified-news-research-2000-3500-v2'}},now),false);
});
