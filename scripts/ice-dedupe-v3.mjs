...
function timeOf(row) { const d = new Date(row.source_created_at || row.event_date || row.last_seen_at || row.first_seen_at || row.published_at || row.created_at || row.updated_at || 0); return Number.isNaN(d.getTime()) ? 0 : d.getTime(); }
function payload(row) { return row?.ai_payload && typeof row.ai_payload === "object" ? row.ai_payload : {}; }
function metadata(row) { return row?.metadata && typeof row.metadata === "object" ? row.metadata : {}; }
function numbers(value) { return new Set((String(value || "").match(/\b\d{1,4}\b/g) || []).map(Number).filter((n) => n > 0 && n !== 2025 && n !== 2026)); }
...
function dimensions(row) {
  ...
  return {
    ...
    family: eventFamily(`${row.event_type || m.event_type || p.event_type || ""} ${facts}`),
    facts
  };
}
...
function sameEvent(a, b, { ignoreTime = false, publishedHistory = false } = {}) {
  ...
  if (!familyCompatible) return { yes: false };
...
function preferred(a, b) {
  const rank = (x) => x.status === "published" ? 6 : x.status === "approved" ? 5 : x.status === "pending_review" ? 4 : x.status === "pending_corroboration" ? 3 : 1;
...
async function main() {
  ...
  for (const story of stories) {
    ...
    let publishedMatch = null, publishedReason = "";
    for (const article of published) {
      const match = sameEvent(story, article, { ignoreTime: true, publishedHistory: true });
      if (match.yes) { publishedMatch = article; publishedReason = match.reason; break; }
    }
    if (publishedMatch && !["published", "rejected"].includes(story.status)) {
      await rest("ice_stories", { method: "PATCH", query: { id: `eq.${story.id}` }, body: { status: "rejected", decision_reason: `自动查重隐藏：${publishedReason}；该事件此前已发布 article=${publishedMatch.id}`, updated_at: new Date().toISOString() }, prefer: "return=minimal" });
