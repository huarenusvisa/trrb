...
// Every source, including official websites, has the same 12-hour ceiling.
...
export function isOlderThanCutoff(row, cutoffMs, nowMs = Date.now()) {
  cutoffMs = Math.max(cutoffMs, nowMs - 12 * 3600000);
...
async function main() {
  ...
  console.log(JSON.stringify({ stage: "ice-drop-stale-posts", requested_max_age_hours: REQUESTED_MAX_AGE_HOURS, enforced_max_age_hours: MAX_AGE_HOURS, official_web_max_age_hours: MAX_AGE_HOURS, scanned: Array.isArray(rows) ? rows.length : 0, deleted: stale.length, cutoff: new Date(cutoffMs).toISOString() }, null, 2));
}
