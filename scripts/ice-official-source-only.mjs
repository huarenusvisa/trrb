...
function isAllowed(row) {
  ...
  const allowedSource = OFFICIAL_HANDLE.test(username) || OFFICIAL_TYPE.test(type) || MONITORED_HANDLE.test(username) || MONITORED_TYPE.test(type);
  return allowedSource && isIceEnforcementEvidence(row?.source_text, username);
}
...
async function reject(ids) {
  for (const group of chunks(ids, 100)) {
    await sb("ice_posts", {
      ...
      body: { relevant: false, processing_status: "irrelevant", last_error: "non_ice_enforcement_source_filtered" },
...
async function main() {
  ...
  const rows = await sb("ice_posts", {
    query: {
      select: "id,source_username,source_display_name,source_type,source_text,processing_status,relevant,source_created_at,created_at",
  ...
  console.log(JSON.stringify({ stage: "strict-original-evidence-filter-v9", scanned: active.length, kept: active.length - rejected.length, rejected_non_ice_enforcement: rejected.length }, null, 2));
}
