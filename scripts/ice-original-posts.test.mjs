...
test("English or overly short story requires Chinese title and body generation", () => {
  ...
  assert.equal(needsTranslation({ title: "缅因州发生ICE执法行动事件", content: body, ai_payload: { translation_version: "zh-title-body-v6-300-600-800-image", translated_to_chinese: true, old_news_checked: true, target_min_chars: 300, target_max_chars: 600 } }, 100, 0), false);
});
...
test("ICE publisher hard-gates Chinese length, image reading, duplicate and old-news checks", () => {
  ...
  assert.match(source, /zh-title-body-v6-300-600-800-image/);
  ...
  assert.match(promoter, /zh-title-body-v6-300-600-800-image/);
