...
async function pendingPosts(){const rows=await sb("ice_posts",{query:{select:"id,x_post_id,source_text,source_type,source_username,source_created_at,created_at,event_fingerprint,event_type,city,state_code,location_text,extraction_payload",processing_status:"in.(collected,processing,extracted,failed)",relevant:"neq.false",order:"source_created_at.desc.nullslast,created_at.desc",limit:String(Number(process.env.ICE_PRECOLLECT_MAX||1200))}});return Array.isArray(rows)?rows:[];}
