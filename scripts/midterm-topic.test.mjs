...
test('中期选举专题从公开文章读取，保留原文链接和历史分页',async t=>{
 ...
 const response=await collection(new Request('https://trrb.net/midterm-elections'),context);const html=await response.text();
 assert.equal(response.status,200);assert.match(html,/<h1>2026中期选举实时动态<\/h1>/);
 assert.match(html,/href="\/us-politics\/senate-0"/);assert.match(html,/rel="next" href="\/midterm-elections\?page=2"/);
 assert.match(html,/rel="canonical" href="https:\/\/trrb.net\/midterm-elections"/);
 const legacy=await redirect(new Request('https://trrb.net/listing.html?q=中期选举'),context);
 assert.equal(legacy.status,301);assert.equal(legacy.headers.get('location'),'https://trrb.net/midterm-elections');
});
