...
const listing=fs.readFileSync('listing.js','utf8');const home=fs.readFileSync('homepage-refresh-guard.js','utf8');const article=fs.readFileSync('article.js','utf8');
...
record(/placeholder|fallback/i.test(article),'文章页具备图片 fallback 逻辑');
...
console.log(`ROUND13 NODE6 audit: checks=${checks.length}; failures=${failures.length}`);if(failures.length){failures.forEach(x=>console.error(`FAIL ${x.label} — ${x.detail}`));process.exit(1);}console.log('ROUND13 NODE6 PASS: image upload and cover failover resilience verified');
