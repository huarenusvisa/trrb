...
async function req(path,method='GET',redirect='follow'){const c=new AbortController();const t=setTimeout(()=>c.abort(),15000);try{return await fetch(`${ORIGIN}${path}${path.includes('?')?'&':'?'}node7=${Date.now()}`,{method,redirect,headers:{'cache-control':'no-cache','user-agent':'TRRB-Round13-Node7/2.0'},signal:c.signal});}finally{clearTimeout(t);}}
...
const sitemap=await req('/sitemap.xml','HEAD');const version=sitemap.headers.get('x-trrb-sitemap')||'';record(/topic-safe/i.test(version)&&/ice-safe/i.test(version),'Sitemap Edge随生产部署可用且启用专题/短ICE保护',version||'missing');
