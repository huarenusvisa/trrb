...
function abs(base, href) { try { return new URL(String(href||'').replace(/&amp;/g,'&'), base).toString(); } catch { return ''; } }
function clean(s='') { return s.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g,'$1').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&#39;/g,"'").replace(/&quot;/g,'"').replace(/&#x2F;/gi,'/').replace(/\s+/g,' ').trim(); }
function links(html, base) {
  ...
  for (const m of html.matchAll(/href\s*=\s*["']([^"']+)["']/gi)) {
    const url=abs(base,m[1]); if(url) out.push({url,text:''});
  }
  return out;
}
function feedLinks(xml, base) {
  const decoded=xml.replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;/g,"'").replace(/&amp;/gi,'&');
  const out=[];
  for (const m of decoded.matchAll(/<link\b[^>]*>([\s\S]*?)<\/link>/gi)) { const url=abs(base,clean(m[1])); if(url) out.push({url,text:''}); }
  for (const m of decoded.matchAll(/<link\b[^>]*href=["']([^"']+)["'][^>]*\/?\s*>/gi)) { const url=abs(base,m[1]); if(url) out.push({url,text:''}); }
  for (const m of decoded.matchAll(/<(?:guid|enclosure)\b[^>]*(?:url=["']([^"']+)["'])?[^>]*>([\s\S]*?)<\/(?:guid|enclosure)>/gi)) { const url=abs(base,m[1]||clean(m[2])); if(url) out.push({url,text:''}); }
  for (const m of decoded.matchAll(/https?:\/\/[^\s<>'"]+/gi)) { const url=abs(base,m[0].replace(/[),.;]+$/,'')); if(url) out.push({url,text:''}); }
  out.push(...links(decoded,base));
  return out;
}
function embeddedUrls(text,base){
  const decoded=text.replace(/&quot;/gi,'"').replace(/&#39;/g,"'").replace(/&amp;/gi,'&').replace(/\\\//g,'/');
  const out=[];
...
function ca6OpinionUrls(text){
  const decoded=text.replace(/&amp;/gi,'&').replace(/\\\//g,'/');
  const out=[];
  for(const m of decoded.matchAll(/(?:https?:\/\/(?:www\.)?opn\.ca6\.uscourts\.gov)?\/?opinions\.pdf\/([0-9]{2}a[0-9]{4}[pn]-06\.pdf)/gi)){
    out.push({url:`https://www.opn.ca6.uscourts.gov/opinions.pdf/${m[1]}`,text:m[1]});
  }
  return out;
}
function ca8OpinionUrls(text){
  const decoded=text.replace(/&amp;/gi,'&'); const out=[];
  for(const m of decoded.matchAll(/(?:href\s*=\s*["']([^"']*?))?((?:\d{6,7})[PUN]\.pdf)/gi)){
    const href=m[1]||m[2]; const url=abs('https://ecf.ca8.uscourts.gov/cgi-bin/TodayOpn.pl',href); if(url) out.push({url,text:m[2]});
  }
  return out;
}
...
function isOpinionDoc(court,x){
  ...
  switch(court.id){
    ...
    case 'ca6': return /(?:^|\.)opn\.ca6\.uscourts\.gov/i.test(new URL(u).hostname)&&/\/opinions\.pdf\/[0-9]{2}a[0-9]{4}[pn]-06\.pdf(?:$|[?#])/i.test(u);
    case 'ca7': return /OpinionsWeb\/processWebInputExternal\.pl/i.test(u);
    case 'ca8': return /(?:^|\.)(?:media|ecf)\.ca8\.uscourts\.gov/i.test(new URL(u).hostname)&&/\d{6,7}[PUN]\.pdf(?:$|[?#])/i.test(u);
...
async function alternateSource(court){
  ...
  for(const url of urls){const r=await get(url);if(r.ok)return r;}return null;
}
