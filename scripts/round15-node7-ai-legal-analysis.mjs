...
const LIMIT=Math.max(3,Math.min(12,Number(process.env.ROUND15_AI_BATCH||8)));
...
async function fetchText(url,id){
  ...
  if(/pdf/i.test(type)||/\.pdf(?:$|[?#])/i.test(r.url)){
    ...
    try{execFileSync('pdftotext',['-layout','-f','1','-l','8',pdf,txt],{stdio:'ignore',timeout:30000});return readFileSync(txt,'utf8').replace(/\s+/g,' ').trim().slice(0,24000)}catch{return''}
  }
  return cleanHtml(await r.text()).slice(0,24000);
}
async function analyze(record,sourceText){
  ...
  const prompt=`你是唐人日报“美国判例与新规”法律资料编辑。必须只依据下方官方一手材料生成中文解析，不得引用或模仿Westlaw、Lexis等商业摘要，不得补充材料中没有的事实，不得把行政命令/行政规则称为国会法律。若材料不足以确认裁判结果，holdingOrRule必须明确写“官方材料节选不足，暂不判断裁判结论”。\n\n记录元数据：${JSON.stringify({sourceSystem:record.sourceSystem,authorityType:record.authorityType,issuingBody:record.issuingBody,title:record.title,citation:record.citation,docket:record.docket,publicationDate:record.publicationDate,officialUrl:record.officialUrl})}\n\n官方材料节选：\n${sourceText}\n\n要求：summary 120-240个汉字；legalIssue说明核心法律问题；holdingOrRule说明裁判结论/行政规则核心内容；impact说明可能影响的对象和范围，并明确区分“直接法律效力”和“可能影响”；sourceGrounding说明解析依据官方来源；disclaimer固定说明“仅供信息参考，不构成法律意见，法律效力以官方原文及后续裁判/规则为准”。只返回JSON。`;
  const res=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:MODEL,input:prompt,max_output_tokens:1800,text:{format:{type:'json_schema',name:'trrb_legal_analysis',strict:true,schema}}})});
