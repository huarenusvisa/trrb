...
node(6,'中文解析完整率、事实约束与缺失回退治理',[
 ...
 ['AI count reconciles',Number(ai.count)===analyses.length],
 ['AI analyses bind to real official records',analyses.every(a=>recordById.has(String(a.recordId)))],
 ['per-analysis versions are current',analyses.every(a=>String(a.datasetVersion||'')===String(db.datasetVersion||''))],
 ['existing analyses are complete',analyses.every(a=>required.every(k=>String(a[k]||'').trim()))],
 ['official metadata is preserved',analyses.every(a=>{const r=recordById.get(String(a.recordId));return r&&['sourceSystem','authorityType','issuingBody','officialUrl','title'].every(k=>String(a[k]??'')===String(r[k]??''))})],
 ['missing analysis falls back to official source',app.includes('请先以官方原文为准')&&detail.includes('中文裁判要旨/规则解析尚未生成')]
]);
...
node(8,'法律栏目生产错误监控、降级与恢复闭环',[
 ...
 ['AI remains optional for official records',app.includes('if(aiRes.ok)')&&detail.includes('if(aiRes.ok)')],
