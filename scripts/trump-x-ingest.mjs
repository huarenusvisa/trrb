...
function clean(value, max = Infinity) { return String(value ?? "").normalize("NFKC").replace(/\u0000/g, "").trim().slice(0, max); }
...
export function targetLength() { return { min: null, max: null, band: "不限字数" }; }
export function isInformational(value, account = "") { const text = sourceText(value); return isOfficialTrumpAccount(account) && Boolean(text) && !/^@/u.test(clean(value)); }
...
function articleSchema() { return { type: "object", additionalProperties: false, required: ["title", "summary", "content", "seo_keywords", "image_observations", "appears_old_news", "old_news_reason"], properties: { title: { type: "string", minLength: 1 }, summary: { type: "string" }, content: { type: "string", minLength: 1 }, seo_keywords: { type: "string" }, image_observations: { type: "string" }, appears_old_news: { type: "boolean" }, old_news_reason: { type: "string" } } }; }
export async function generateChineseDraft(candidate, attempt = 0, previous = null) {
  ...
  const response = await request("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: OPENAI_MODEL, store: false, max_output_tokens: 2200, instructions: ["你是唐人日报特朗普专题编辑。所有字段必须使用简体中文；原文为英文时必须完整翻译和编辑，禁止输出完整英文句子。人名可保留常用英文拼写，ICE、DHS等缩写可保留。", "正文不设字数上下限，按原帖实际信息完整准确地写作，短讯保持简洁，不为字数扩写或截断事实。只依据原帖文字和图片可见信息写作，严禁编造人物、数字、地点、引语、原因、结果或外部背景。", "对观点、指控和预测明确归因于发帖账号，不得写成已证实事实。过滤辱骂、口号、纯立场表达和缺乏新闻事实的评论。", "如有图片，必须逐张读取可辨认的文字、人物、地点、标志、物件、数量、颜色和动作，并以‘画面可见’或‘图片文字显示’归因；看不清不写。视频只按静态缩略图处理。", "不得根据外貌推断身份、职业、族群、健康、犯罪倾向或动机。不得用提醒、呼吁、空泛评价、重复句或免责声明凑字。", "判断原帖是否明确在回顾旧事件；只有原文明确写出过去日期、周年、回顾、旧视频或旧照片时，appears_old_news才为true，并在old_news_reason说明证据。不得凭模型记忆判断。", "标题准确概括与特朗普直接相关的新闻事实；summary简洁概括实际信息，不为摘要凑字；content只放正文，不用Markdown。image_observations用中文简述实际读取到的画面信息，无图或没有可辨信息则留空。", previous ? `上一版标题或正文为空，或未通过中文检查，请重写。上一版正文：${clean(previous.content, 3_000)}` : ""].filter(Boolean).join("\n"), input: [{ role: "user", content: [{ type: "input_text", text: `原帖账号：${candidate.source_account || "未知"}\n原帖时间：${candidate?.raw_payload?.source_created_at || "未知"}\n原帖文字：\n${raw}\n\n请翻译、读图并编辑为中文新闻稿。` }, ...images] }], text: { format: { type: "json_schema", name: "trump_x_chinese_draft", strict: true, schema: articleSchema() } } }) });
  const parsed = JSON.parse(responseText(response)); for (const key of ["title", "summary", "content", "seo_keywords", "image_observations", "old_news_reason"]) parsed[key] = clean(parsed[key]);
  if ((!parsed.title || !parsed.content || !hasChinese(parsed.title) || chineseRatio(parsed.content) < 0.45) && attempt < 3) return generateChineseDraft(candidate, attempt + 1, parsed);
  if (!parsed.title || !parsed.content) throw new Error("生成稿标题和正文不能为空，禁止发布");
  if (!hasChinese(parsed.title) || chineseRatio(parsed.content) < 0.45) throw new Error("生成稿未通过中文检查，禁止发布");
...
export async function processCandidate(candidate, articles) {
  if (!isInformational(candidate.raw_text, candidate.source_account)) { await patchCandidate(candidate.id, { decision: "rejected", decision_reason: "不是特朗普本人原创帖，或原帖正文为空，不进入编辑发布流程", processed_at: new Date().toISOString(), ai_payload: { ...(candidate.ai_payload || {}), status: "filtered_non_official_or_empty" } }); return "rejected"; }
  try {
  ...
  } catch (error) { await patchCandidate(candidate.id, { decision: "review_required", decision_reason: `自动翻译、读图、查重或内容检查未通过：${clean(error?.message || error, 600)}；不得直接发布英文原文`, processed_at: new Date().toISOString(), ai_payload: { ...(candidate.ai_payload || {}), status: "review_required", automatic_publish_blocked: true } }); return "review_required"; }
}
