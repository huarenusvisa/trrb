...
function isArticleUrl(value){
  try{
    ...
    if(!['trrb.net','www.trrb.net'].includes(u.hostname))return false;
...
async function collectArticleUrls(){
  ...
  while(queue.length && seen.size<30 && articles.size<SAMPLE){
    const url=queue.shift();
    if(seen.has(url)) continue;
    seen.add(url);
    let res;
    try{res=await fetch(url,{redirect:'follow',headers:{'user-agent':UA_GOOGLE,'accept':'application/xml,text/xml,*/*','cache-control':'no-cache'}});}catch{continue;}
    if(!res.ok) continue;
    const xml=await res.text();
    for(const a of articleLocs(xml)){articles.add(a);if(articles.size>=SAMPLE)break;}
    for(const loc of allLocs(xml)){
      if(articles.size>=SAMPLE) break;
      try{
        const u=new URL(loc);
        if(u.hostname!=='trrb.net'&&u.hostname!=='www.trrb.net') continue;
        if(/\.xml(?:$|\?)/i.test(u.pathname+u.search) || /sitemap/i.test(u.pathname)) queue.push(loc.replace('https://www.trrb.net',ORIGIN));
      }catch{}
    }
  }
...
if(urls.length<3){throw new Error(`sitemap pretty-article sample too small: ${urls.length}; checked: ${collected.seen.join(', ')}`);}
...
for(const url of urls){
  for(const [agent,ua] of [['googlebot',UA_GOOGLE],['browser',UA_BROWSER]]){
    ...
    details.push({url,agent,status:res.status,canonical,title,h1,bodyLength:bodyText.length,prerenderHeader,robots,bad});
    if(bad.length) failures.push({url,agent,bad});
  }
}

// Confirm the legacy UUID query path no longer serves a crawlable duplicate shell.
const samplePath=new URL(urls[0]).pathname.split('/').filter(Boolean);
const sampleSlug=samplePath[1]||'';
if(/^[0-9a-f]{8}-[0-9a-f-]{27,}$/i.test(sampleSlug)){
  const legacy=`${ORIGIN}/article.html?id=${encodeURIComponent(sampleSlug)}`;
  const legacyRes=await get(legacy,UA_GOOGLE,'manual');
  const location=legacyRes.headers.get('location')||'';
  if(!(legacyRes.status===301&&location===urls[0])) failures.push({url:legacy,agent:'googlebot',bad:['legacy-uuid-not-one-hop-canonical'],status:legacyRes.status,location});
}
...
console.log(`预渲染线上验证：${urls.length}篇漂亮canonical × 2种UA；失败 ${failures.length}`);
