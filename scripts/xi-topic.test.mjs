...
test('topic renders crawlable published news with canonical pagination and escaped content', async t => {
  setup(t);
  const res = await xiTopic(new Request('https://trrb.net/topic/xi-jinping'), context);
  ...
  assert.match(body, /rel="canonical" href="https:\/\/trrb.net\/topic\/xi-jinping"/);
  assert.match(body, /rel="next" href="\/topic\/xi-jinping\?page=2"/);
...
test('last page has own canonical and previous link, without fabricated next page', async t => {
  setup(t, 2);
  const body = await (await xiTopic(new Request('https://trrb.net/topic/xi-jinping?page=6'), context)).text();
  assert.match(body, /rel="canonical" href="https:\/\/trrb.net\/topic\/xi-jinping\?page=6"/);
  assert.match(body, /rel="prev" href="\/topic\/xi-jinping\?page=5"/);
  assert.doesNotMatch(body, /rel="next"/);
});
test('empty pages and upstream failures do not become indexable empty shells', async t => {
  setup(t, 0);
  const res = await xiTopic(new Request('https://trrb.net/topic/xi-jinping?page=999'), context);
  assert.equal(res.status, 404);
  assert.match(res.headers.get('x-robots-tag'), /noindex/);
  t.mock.restoreAll();
  setup(t, 0, true);
  const failed = await xiTopic(new Request('https://trrb.net/topic/xi-jinping'), context);
...
test('HEAD remains bodyless and aliases redirect to the same page', async t => {
  setup(t, 1);
  const head = await xiTopic(new Request('https://trrb.net/topic/xi-jinping', {method:'HEAD'}), context);
  assert.equal(head.status, 200);
  assert.equal(await head.text(), '');
  const alias = await xiTopic(new Request('https://trrb.net/topic/xi-jinping/?page=2'), context);
  assert.equal(alias.status, 301);
  assert.equal(alias.headers.get('location'), 'https://trrb.net/topic/xi-jinping?page=2');
});
