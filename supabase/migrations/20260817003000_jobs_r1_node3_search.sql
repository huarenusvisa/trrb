...
-- JOBS-R1-N3: public US-only job search over the same canonical job_listings table.
-- This function is SECURITY INVOKER so existing RLS remains authoritative.
create index if not exists job_listings_search_title_idx
  on public.job_listings using gin (to_tsvector('simple', coalesce(title,'') || ' ' || coalesce(description,'')));
create index if not exists job_listings_search_geo_idx
  on public.job_listings(state_code, city, county, borough, neighborhood, status, published_at desc);
create index if not exists job_listings_search_salary_idx
  on public.job_listings(salary_min, salary_max) where status='open';
...
as $$
  ...
  select
    ...
    f.state_code,f.city,f.county,f.borough,f.neighborhood,
...
grant execute on function public.search_job_listings(text,text,text,text,text,text,text,text,numeric,text,numeric,numeric,integer,integer) to anon, authenticated;

comment on function public.search_job_listings is
  'JOBS-R1-N3 US-only public job search. Uses canonical job_listings under RLS; supports hierarchy, keyword, category, employment type, salary, relevance/latest/salary/distance ordering.';
