...
-- JOBS-R3 N2: keep the canonical R2 search RPC, adding a blue-collar-first sort.
create or replace function public.search_job_listings(
  ...
  p_sort text default 'blue_collar',
...
returns table (
  id uuid,title text,description text,category_slug text,employment_type text,
  salary_min numeric,salary_max numeric,salary_period text,state_code text,city text,
  county text,borough text,neighborhood text,postal_code text,latitude numeric,longitude numeric,
  published_at timestamptz,created_at timestamptz,distance_miles numeric,relevance_score real
)
language sql stable security invoker set search_path = public
as $$
  with filtered as (
    select j.*,
      case when nullif(btrim(coalesce(p_keyword,'')), '') is null then 0::real
        else ts_rank(to_tsvector('simple',coalesce(j.title,'')||' '||coalesce(j.description,'')),plainto_tsquery('simple',btrim(p_keyword))) end as score,
      case when p_latitude is null or p_longitude is null or j.latitude is null or j.longitude is null then null::numeric
  ...
  select f.id,f.title,f.description,f.category_slug,f.employment_type,f.salary_min,f.salary_max,f.salary_period,
    f.state_code,f.city,f.county,f.borough,f.neighborhood,f.postal_code,f.latitude,f.longitude,f.published_at,f.created_at,f.miles,f.score
  from filtered f
  ...
  order by
    ...
    case when p_sort='blue_collar' then case f.category_slug
...
comment on function public.search_job_listings(text,text,text,text,text,text,text,text,text,numeric,text,numeric,numeric,numeric,integer,integer) is
  'JOBS-R3 canonical US-only search: R2 location behavior plus blue-collar-first marketplace ordering.';
