...
begin
  ...
  if new.category_name = '移民美国'
     ...
     and not us_immigration_match then
...
where lower(c.slug) = 'ice'
  ...
  and (coalesce(a.title, '') || ' ' || coalesce(a.summary, ''))
      ~* '(抓捕|抓获|拘捕|逮捕|拘留|拘押|羁押|遣返|递解|驱逐出境|强制离境|突袭|搜捕|通缉|扫荡|执法行动|拘留令|扣押令|arrest|detain|detention|deport|deportation|removal|raid|custody|fugitive|warrant)';
